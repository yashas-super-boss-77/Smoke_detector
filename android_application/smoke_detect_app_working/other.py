...
class TouchVideo(Video):
    ...
    def draw_lines(self):
        ...
        # vertical_line_x = self.x 

        print (vertical_line_x, self.y/2)
        self.lines.extend([vertical_line_x, max(self.y*2, 0), vertical_line_x, self.top])

        # Horizontal line dividing the frame into top and bottom halves
        horizontal_line_y = self.y + self.height / 2
        # horizontal_line_y = self.y
        self.lines.extend([max(self.x, 0), horizontal_line_y, self.right, horizontal_line_y])
