...
class VideoPlayerApp(App):
    ...
    def build(self):
        ...
        # Video
        self.video = TouchVideo(size_hint=(1, 0.8))  # Increase the size of the video display box
        layout.add_widget(self.video)

        # Button to browse video
        browse_button = Button(text='Browse', size_hint=(1, 0.1))  # Set smaller size for the button
        browse_button.bind(on_press=self.browse)
        layout.add_widget(browse_button)

        # Button to pause/resume video
        self.pause_button = Button(text='Pause', size_hint=(1, 0.1))  # Set smaller size for the button
        self.pause_button.bind(on_press=self.pause_resume_video)
        layout.add_widget(self.pause_button)

        # Button to detect objects
        self.detect_button = Button(text='Detect Objects', size_hint=(1, 0.1))  # Set smaller size for the button
        ...
        self.objects_label = Label(text='Detected Objects:')
